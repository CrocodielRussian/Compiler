...
module StatementParser = struct
  ...
  type statement =
    | Assign of string * asign_oper * ExpressionParser.expr
    | Empty
  ...
  let identifier text pos =
    let acc = ref "" in
    while !pos < String.length text && ExpressionParser.is_alpha text.[!pos] do
      acc := !acc ^ String.make 1 text.[!pos];
      incr pos
    done;
    !acc
  ...
  let variable text pos =
    ExpressionParser.skip_whitespaces text pos;
    let acc = identifier text pos in
    if String.length acc > 0 then ExpressionParser.Variable acc
    else
      failwith
        ("Parser Error: on position " ^ (!pos |> string_of_int)
  ...
  let stmt_to_string text pos = function
    ...
    | Assign (e1, op, e2) ->
        ...
        in
        "<var: " ^ e1 ^ "> " ^ op_str ^ " "
        ^ ExpressionParser.expr_to_string text pos e2
        ^ ";"
  ...
  let stmt text pos =
    ...
    if !pos >= String.length text then Empty
      ...
      pos := !pos + 2;
      let asign =
        Assign (ident, operation, ExpressionParser.parse_expr text pos)
      in
